...
| **4. 姓名·电话修改** | 我的资料 → 注册信息 → 编辑；姓名用罗马字/拼音，支付渠道（如 Peska）要求与注册一致 | 擅自代客户修改或与证件不一致 |
| **5. 重置密码** | 仅通过系统「忘记密码」链接；邮箱变更 + 密码重置时，需先完成邮箱变更再引导客户在 Portal 修改 | 口头/截图/邮件发送新密码 |
| **6. 杠杆修改** | **必须先确认无持仓**；修改完成后告知账号 ID 与新杠杆倍数 | 未清仓即修改（易导致爆仓） |
| **7. 故障排查** | 三步法：确认现象 → 复现与隔离 → 升级转 IT；记录工单号与预计回复时间 | 无记录即升级；承诺「马上修好」 |
| **8. MT5 模拟账户** | 与真实账户为不同场景；引导客户在 MT5 内自助创建，服务器：`AsiaFutureTrading-Server` | 替客户代为创建 |
| **9. Zendesk 文档** | 官网帮助中心链接汇总，转发时须按客户语言选用对应版本 | 对日文/中文客户发英文链接 |
...
| 4 | 姓名·电话修改 (Name & Phone Update) | [4. 姓名·电话修改](plan.html#01-4) |
| 5 | 重置密码 (Password Reset / パスワード再設定) | [5. 重置密码](plan.html#01-5) |
| 6 | 杠杆修改 (Leverage Change / レバレッジ変更) | [6. 杠杆修改](plan.html#01-6) |
| 7 | 故障排查三步法 (Troubleshooting) | [7. 故障排查](plan.html#01-7) |
| 8 | MT5 模拟账户自助创建 (Demo Account / デモ口座) | [8. MT5 模拟账户](plan.html#01-8) |
| 9 | Zendesk 文档中心快速索引 | [9. Zendesk 文档](plan.html#01-9) |
...
→ 完整文章列表见 [9. Zendesk 文档](plan.html#01-9)
